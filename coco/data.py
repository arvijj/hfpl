...
def load_image_label_from_ann(img_name, coco_root):
    ann_img = cv2.imread(os.path.join(coco_root, ANNOT_FOLDER_NAME, img_name + '.png'))
...
def load_image_label_list_from_anns(img_name_list, coco_root):
    return [load_image_label_from_ann(img_name, coco_root) for img_name in img_name_list]

def load_image_label_list_from_npy(img_name_list, coco_root):
    cls_labels_dict = np.load(os.path.join(coco_root, 'cls_labels.npy'), allow_pickle=True).item()
...
class COCOClsDataset(COCOImageDataset):
    ...
    def __init__(self, img_name_list_path, coco_root, transform=None):
        ...
        self.label_list = load_image_label_list_from_npy(self.img_name_list, coco_root)
        #self.label_list = load_image_label_list_from_anns(self.img_name_list, self.coco_root)
...
class COCOClsDatasetMS(COCOClsDataset):
...
class COCOAffDataset(COCOImageDataset):
    ...
    def __init__(self, img_name_list_path, label_la_dir, label_ha_dir, cropsize, coco_root, radius=5,
...
class COCOAffGtDataset(COCOImageDataset):

    def __init__(self, img_name_list_path, label_dir, cropsize, coco_root, radius=5,
                 joint_transform_list=None, img_transform_list=None, label_transform_list=None):
        super().__init__(img_name_list_path, coco_root, transform=None)
