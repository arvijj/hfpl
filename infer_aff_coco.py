...
if __name__ == '__main__':
    ...
    parser.add_argument("--infer_list", default="coco/val2014.txt", type=str)
    ...
    infer_dataset = coco.data.COCOImageDataset(args.infer_list, coco_root=args.coco_root,
    ...
    for iter, (name, img) in enumerate(infer_data_loader):
        ...
        cam_full_arr = np.zeros((81, orig_shape[2], orig_shape[3]), np.float32)
...
#                cam_full_arr = torch.from_numpy(cam_full_arr).view(1, 81, dheight, dwidth).cuda()
            cam_vec = cam_full_arr.view(81, -1)

            cam_rw = torch.matmul(cam_vec.cuda(), trans_mat)
            cam_rw = cam_rw.view(1, 81, dheight, dwidth)
            ...
            if args.crf:    
                ...
                cam_rw = torch.from_numpy(cam_rw).view(1, 81, img.shape[2], img.shape[3]).cuda()
            ...
            os.makedirs(args.out_rw, exist_ok=True)
